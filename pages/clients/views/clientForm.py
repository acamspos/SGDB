...
from assets.globals import check_email_format, on_validate_length


class ClientForm(ttb.Toplevel):
    ...
    def __createWidgets(self):
        ...
        self.rifEntry = ttb.Entry(mainInforFrame, textvariable=self.__rif, width=30,validatecommand=(self.register(lambda e: on_validate_length(e,lenght=20)), '%P'))
        ...
        self.nameEntry = ttb.Entry(mainInforFrame, width=80, textvariable=self.__name,validatecommand=(self.register(lambda e: on_validate_length(e,lenght=200)), '%P'))
        ...
        self.addressEntry = ttb.Entry(mainInforFrame, textvariable=self.__address, width=50,validatecommand=(self.register(lambda e: on_validate_length(e,lenght=400)), '%P'))
        ...
        email_label = ttb.Label(moreInfoFrame, 
                                     ...
                                     background=FGCOLOR ,
                                     font=('arial',11,'bold'))
        email_label.grid(row=0, column=0, padx=(4,4), pady=(2,0), ipadx=8, ipady=8, sticky='nsew')
        self.emailEntry = ttb.Entry(moreInfoFrame, textvariable=self.__email, validatecommand=(self.register(lambda e: on_validate_length(e,lenght=200)), '%P'))
        self.emailEntry.grid(row=1, column=0, sticky='nsew',padx=(4,4), ipady=4,)
        ...
        self.phoneNumberEntry = ttb.Entry(moreInfoFrame, textvariable=self.__phone,validatecommand=(self.register(lambda e: on_validate_length(e,lenght=12)), '%P') )
        ...
        self.websiteEntry = ttb.Entry(moreInfoFrame, textvariable=self.__website,validatecommand=(self.register(lambda e: on_validate_length(e,lenght=200)), '%P'))
        self.websiteEntry.grid(row=3, column=0, sticky='nsew',padx=(4,0), ipady=4,columnspan=2)
        ...
        if self.window_type == 'create':
            ...
            self.createBTN = ButtonImage(buttonss_section_frame,  command=self.createClientRecord, image=self.creatbtnimg, img_h=self.creatbtnimgh, img_p=self.creatbtnimgp, style='flatw.light.TButton', text='REGISTRAR', compound='center',padding=0)
    ...
    def __check_fields(self):
        if not check_email_format(self.__email.get()):
            messagebox.showwarning('Correo','La informacion ingresada no es un correo Electronico')
            return False
        if '' in [value.get() for value in self.FORM_DATA]:
            return False
        return True
    

    def createClientRecord(self):
        if self.__check_fields():
            if Client.validate_rif(self.__rif.get()):
                ask = messagebox.askquestion('Crear','Crear Registro de Cliente?', parent=self)
                if ask == 'yes':
                    newclient = self.clientInstance()
                    newclient.create()
    ...
    def editClientRecord(self):
        if self.__check_fields():
            ask = messagebox.askquestion('Modificar','Modificar el Registro de Cliente?', parent=self)
            if ask == 'yes':
                newclient = self.clientInstance()
                newclient.update()
...
# SGDB_Style()
# ClientForm(window_type='create', title='Crear Cliente')
# app.mainloop()