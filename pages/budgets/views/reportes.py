...
class Report(ttb.Toplevel):
    def __init__(self, master=None, window_type = 'create', title = '', product = None):
        ...
        self.fileNameVar = ttb.StringVar(value=f"Reporte_de_Cotizaciones_{datetime.today().strftime('%d-%m-%Y')}")
    ...
    def set_provider_rif(self, rif):
        self.__CLIENT_RIF.set(rif.rif)
        self.fileNameVar.set(f"{rif.rif}_"+self.fileNameVar.get())

    def clean_provider_field(self):
      
        if self.__CLIENT_RIF.get():
            self.fileNameVar.set(self.fileNameVar.get().replace(f"{self.__CLIENT_RIF.get()}_",''))
            self.__CLIENT_RIF.set('')

    def open_provider_selection(self):
        window = ClientModule(self,selectionMode=True, callback=self.set_provider_rif)


    def get_sql_report_sentence(self):
        sql = """SELECT b.code as Codigo, b.description as Descripcion, cl.name as Cliente, b.address as Direccion, r.name as Representante ,b.creationDate as 'Fecha de Creacion',
        b.processingDate as 'Fecha de Procesamiento', b.finishDate as 'Fecha de Finalizacion',c.description as Moneda, s.description as Status, b.sub_total as 'Sub Total', b.iva as Iva,
        b.total_amount as Total, (CASE WHEN b.currency=2 THEN b.total_amount ELSE b.total_amount/b.exchange_rate END) AS "Total USD"
        FROM budgets b
        INNER JOIN clients cl ON cl.rif = b.client
        INNER JOIN currency c ON c.id=b.currency
        INNER JOIN representative r ON r.id=b.representative
        INNER JOIN status s ON s.id = b.state
        """
        ...
        if payment_status:
            extra_fields.append(f'b.state = {payment_status}')
        
        

        rif = self.__CLIENT_RIF.get()
        if rif:
            extra_fields.append(f"client = '{rif}'")
    ...
    def styling_report(self, path_file:str = ''):
        ...
        set_width_to(sheet, "A", "N", width=30)
