...
class ProviderView(ttb.Toplevel):
    ...
    def __createWidgets(self):
        ...
        roundFrameContainer = CTkFrame(self, fg_color='white',border_width=1,  border_color='#CFCFCF')
        roundFrameContainer.grid(row = 0, column = 0, sticky = 'nsew', padx=10, pady=10)
        roundFrameContainer.columnconfigure(0, weight=1)
        roundFrameContainer.rowconfigure(2, weight=1)


        self.__PROVIDERICON = resize_icon(Image.open(f"{IMG_PATH}/companyicon.png"))


        titleFrame = CTkFrame(roundFrameContainer, fg_color='#212946')
        titleFrame.grid(row = 0, column = 0, sticky = 'nsew', padx=8, pady=(8,0))
        titleFrame.columnconfigure(0, weight=1)
        
        ttb.Label(titleFrame, image=self.__PROVIDERICON, compound='left', text=f' {self.window_title}', font=(GUI_FONT, 14, 'bold'), 
                  foreground='white', background='#212946').grid(row=0,column=0, padx=10, pady=10, sticky='nsew')

        buttons_frame = ttb.Frame(roundFrameContainer, style='white.TFrame')
        ...
        self.MAIN_FRAME = tk.Frame(roundFrameContainer)
    ...
    def secondPage(self):
        ...
        self.__document_2_pay_gridview.heading(columns[0], anchor='center', text='Fecha')
        self.__document_2_pay_gridview.heading(columns[1], anchor='w', text='Documento')
        self.__document_2_pay_gridview.heading(columns[2], anchor='w', text='Control')
        self.__document_2_pay_gridview.heading(columns[3], anchor='w', text='Vencimiento')
        self.__document_2_pay_gridview.heading(columns[4], anchor='w', text='Estatus')
        self.__document_2_pay_gridview.heading(columns[5], anchor='w', text='Moneda')
        self.__document_2_pay_gridview.heading(columns[6], anchor='w', text='Total $')
        self.__document_2_pay_gridview.heading(columns[7], anchor='w', text='T. Pagado $')
        self.__document_2_pay_gridview.heading(columns[8], anchor='w', text='T. Deuda $')
        self.__document_2_pay_gridview.heading(columns[9], anchor='w', text='Tasa de Cambio Bs.')
        self.__document_2_pay_gridview.heading(columns[10], anchor='w', text='T. Moneda Principal')
        

        self.__document_2_pay_gridview.column(columns[0], width=0, stretch=False, anchor='center')
        self.__document_2_pay_gridview.column(columns[1], width=140, stretch=False, anchor='w')
        self.__document_2_pay_gridview.column(columns[2], width=140, stretch=False, anchor='w')
        self.__document_2_pay_gridview.column(columns[3], width=140, stretch=False, anchor='w')
        self.__document_2_pay_gridview.column(columns[4], width=140, stretch=False, anchor='w')
        self.__document_2_pay_gridview.column(columns[5], width=140, stretch=False, anchor='w')
        self.__document_2_pay_gridview.column(columns[6], width=140, stretch=False, anchor='w')
        self.__document_2_pay_gridview.column(columns[7], width=140, stretch=False, anchor='w')
        self.__document_2_pay_gridview.column(columns[8], width=140, stretch=False, anchor='w')
        self.__document_2_pay_gridview.column(columns[9], width=160, stretch=False, anchor='center')
        self.__document_2_pay_gridview.column(columns[10], width=140, stretch=False, anchor='center')
    ...
    def thirthPage(self):
        ...
        self.__document_all__gridview.heading(columns[0], anchor='center', text='Fecha')
        self.__document_all__gridview.heading(columns[1], anchor='center', text='Documento')
        self.__document_all__gridview.heading(columns[2], anchor='center', text='Control')
        self.__document_all__gridview.heading(columns[3], anchor='center', text='Vencimiento')
        self.__document_all__gridview.heading(columns[4], anchor='center', text='Estatus')
        self.__document_all__gridview.heading(columns[5], anchor='center', text='Moneda')
        self.__document_all__gridview.heading(columns[6], anchor='center', text='Total $')
        self.__document_all__gridview.heading(columns[7], anchor='center', text='T. Pagado $')
        self.__document_all__gridview.heading(columns[8], anchor='center', text='T. Deuda $')
        self.__document_all__gridview.heading(columns[9], anchor='center', text='Tasa de Cambio\nBs.')
        self.__document_all__gridview.heading(columns[10], anchor='center', text='T. Moneda \nPrincipal')
        ...
        self.__document_all__gridview.column(columns[8], width=140, stretch=False, anchor='center')
        self.__document_all__gridview.column(columns[9], width=140, stretch=False, anchor='center')
        self.__document_all__gridview.column(columns[10], width=140, stretch=False, anchor='center')
    ...
    def __set_provider_info(self):
        ...
        self.set_documents(self.__document_2_pay_gridview, self.__PROVIDER.get_debit_documents)
        self.set_documents(self.__document_all__gridview, self.__PROVIDER.get_all_documents)
        self.set_payments()
        self.update_balance()
    ...
    def set_documents(self, table = None, documentsFunc = None):
        ...
        if self.__PROVIDER:
            ...
            for document in documents:
                purchase = PurchaseDocument(**document)

       

                totalMain = float(purchase.total)
                totalPaidUSD = round(float(purchase.totalPaidUSD), 2)
                debtUSD = round(float(purchase.debtUSD), 2)
                totalUSD = float(purchase.totalUSD)
  
                table.insert("",
                        ttb.END,
                        values=(purchase.registrationDate.strftime('%d/%m/%Y'), purchase.code, purchase.control, 
                            purchase.expirationDate.strftime('%d/%m/%Y'), purchase.get_documentState(), 
                            purchase.get_currency(), totalUSD, totalPaidUSD, debtUSD, purchase.exchangeRate, totalMain), 
                        )
    ...
    def set_payments(self):
        ...
        if self.__PROVIDER:
            ...
            payments = self.__PROVIDER.get_payments_records(purchase=self.paymentsSearchEntry.get())
    ...
    def update_balance(self):
        self.totalDebtVar.set(self.__PROVIDER.get_total_debt())
        self.actualDebtVar.set(self.__PROVIDER.get_actual_debt())
        self.totalPaidVar.set(self.__PROVIDER.get_total_paid())              

    def payment(self):
        documentSelected = self.__document_2_pay_gridview.focus()
        if documentSelected:
            code = self.__document_2_pay_gridview.item(documentSelected, 'values')[1]
            document = PurchaseDocument.findOnePurchase(code, rif=self.__PROVIDER.rif)
            paymentWindow = PaymentForm(self, doc=document, callback = self.udpate_debt_balance)
...
# SGDB_Style()
# ttb.Button(text='HELLO', command=lambda:ProviderView(window_type='view', title='Detalles Proveedor')).pack(padx=20, pady=20)
# app.mainloop()